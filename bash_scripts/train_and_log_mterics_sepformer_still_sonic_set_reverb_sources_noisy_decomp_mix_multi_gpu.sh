...
echo "train sepformer still_sonic_set_v1_reverb_sources_noisy_decomp_mix dataset using 1 GPUS"
nvidia-smi
...
/home/afrumme1/miniconda3/envs/speechbrain/bin/torchrun --nproc_per_node=4 /export/fs05/afrumme1/git/speechbrain/recipes/WSJ0Mix/separation/train_with_metrics_log.py /export/fs05/afrumme1/git/speechbrain/recipes/WSJ0Mix/separation/hparams_copy/sepformer_log_metrics-still_sonic_set_decomp_mixtire_rev_sources_multi_gpu.yaml --data_folder /export/fs05/afrumme1/sepformer_training/still_sonic_set_v1_reverb_sources_noisy_decomp_mix


# Capture exit status
if [ $? -eq 0 ]; then
    echo "train sepformer on noisy-still-sonic-set mixture dataset using 1 GPUS completed successfully!"
else
